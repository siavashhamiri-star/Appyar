...
class CalculateStorageAdjustmentUseCase(
    ...
    suspend operator fun invoke(
        ...
        val activeStorageAgreements = allAgreements.filter { agreement ->
            agreement.status == AgreementStatus.ACTIVE &&
            (agreement.relatedStorageId != null || agreement.agreementType.name.startsWith("STORAGE"))
        }

        val chargesByUnitId = allCharges.associateBy { it.unitId }.toMutableMap()

        for (agreement in activeStorageAgreements) {
            ...
            val sourceNote = "${sourceCharge.calculationNotes ?: ""}\n[تعدیل توافق انباری]: کسر $adjustmentValue به دلیل واگذاری انباری".trim()
            ...
            // 2. Target Unit receives positive adjustment for storage use
            ...
            val targetNote = "${targetCharge.calculationNotes ?: ""}\n[تعدیل توافق انباری]: اضافه شدن $adjustmentValue بابت استفاده از انباری".trim()
            ...
            auditRepository.recordEvent(
                AuditEvent(
                    actor = userId,
                    action = "CALCULATE_STORAGE_ADJUSTMENT",
                    entity = "Agreement",
                    entityId = agreement.id,
                    buildingId = buildingId,
                    details = "اعمال اثر مالی توافق انباری به مبلغ $adjustmentValue بر شارژ واحدهای ${sourceCharge.unitId} و ${targetCharge.unitId}"
                )
            )
        }
